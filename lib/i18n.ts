      ...
      items: [
        {
          ...
          bullets: [
            ...
            "Built **Pulse**, a self-hosted uptime monitor in production on **Node 22 + TypeScript (strict, no transpilation) + Fastify 5 + PostgreSQL 16 + BullMQ/Redis 7**, deliberately **without an ORM** — 22 versioned `.sql` files so every query stays readable and `EXPLAIN`-able. Sustains **1,674 req/s at p99 38ms** on the listing route (autocannon, 3 rounds of 15s at 50 connections, p50/p95/p99 recorded, never an average), and I identified the expensive route by measurement rather than assumption: the detail view runs **13× slower** at 127 req/s / p99 477ms, with its `EXPLAIN` baseline versioned before any optimisation. Diagnosed aggregation cost with **EXPLAIN (ANALYZE, BUFFERS)** over **1,501,920 rows** — the 24h summary touches **4,812 buffers to return 3 numbers** against 6 for the listing — and found planning cost growing with partition count, which turns `DROP TABLE` retention into a performance requirement rather than a disk one.",
            "Engineered the concurrency and security layer of **Pulse**: distributed self-scheduling in **BullMQ** where each check enqueues the next with **0–5000ms jitter** (a central cron would stampede every monitor on the :00 second), and **exactly-once execution without a distributed lock** by deriving the jobId from the persisted `next_check_at` — proven by a test running two reconcilers in parallel. Notification idempotency uses a short-TTL **Redis** reservation promoted only after send, so a mid-crash expires and retries while a failed send releases immediately. Closed a full-surface **SSRF** in a product whose job is fetching user-supplied URLs: single timed DNS resolution, IP validated against `net.BlockList` (loopback, RFC1918, link-local/metadata, CGNAT, ULA) and the resolved address handed to the socket, so validator and connector see the same IP — which closes **DNS rebinding**. Password KDF is **scrypt** with `timingSafeEqual`, hashing even on unknown emails so login timing cannot enumerate accounts. **41 integration tests against real PostgreSQL and Redis, no mocks of my own code**, green in CI — on **9 runtime dependencies**, with no JWT, password-hashing or i18n library.",
      ...
      items: [
        {
          ...
          bullets: [
            ...
            "Desenvolvi o **Pulse**, monitor de disponibilidade self-hosted em produção sobre **Node 22 + TypeScript (strict, sem transpilação) + Fastify 5 + PostgreSQL 16 + BullMQ/Redis 7**, deliberadamente **sem ORM** — 22 arquivos `.sql` versionados, mantendo toda query legível e passível de `EXPLAIN`. Sustenta **1.674 req/s com p99 de 38ms** na rota de listagem (autocannon, 3 rodadas de 15s a 50 conexões, p50/p95/p99 registrados, nunca média), e identifiquei a rota cara por medição, não por suposição: a visão de detalhe roda **13× mais lenta**, a 127 req/s e p99 de 477ms, com o `EXPLAIN` do \"antes\" versionado antes de qualquer otimização. Diagnostiquei o custo de agregação com **EXPLAIN (ANALYZE, BUFFERS)** sobre **1.501.920 linhas** — o resumo 24h toca **4.812 buffers para devolver 3 números**, contra 6 da listagem — e constatei o custo de planejamento crescendo com o número de partições, o que transforma retenção por `DROP TABLE` em requisito de performance, não de disco.",
            "Construí a camada de concorrência e segurança do **Pulse**: auto-agendamento distribuído em **BullMQ** onde cada verificação enfileira a próxima com **jitter de 0 a 5000ms** (um cron central dispararia todos os monitores no segundo :00), e **exatamente-uma-execução sem lock distribuído**, derivando o jobId do `next_check_at` persistido — provado por teste que roda dois reconciliadores em paralelo. A idempotência de notificação usa reserva de TTL curto em **Redis** promovida a definitiva só após o envio: crash no meio expira e o retry envia, envio que falha libera na hora. Fechei um **SSRF** de superfície completa num produto cuja função é buscar URLs que o usuário cola: resolução de DNS única e cronometrada, IP validado contra `net.BlockList` (loopback, RFC1918, link-local/metadata, CGNAT, ULA) e entrega do endereço já resolvido ao socket, de modo que quem valida e quem conecta olham o mesmo IP — o que fecha **DNS rebinding**. O KDF de senha é **scrypt** com `timingSafeEqual`, mantendo o hash mesmo com e-mail inexistente, para o tempo de login não enumerar contas. **41 testes de integração contra PostgreSQL e Redis reais, sem mock de código próprio**, verdes em CI — sobre **9 dependências de runtime**, sem biblioteca de JWT, de hash de senha ou de i18n.",
